                ...
                <tbody>
                  ...
                  <tr class="bg-gray-100 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200/70 dark:hover:bg-gray-800/60">
                    ...
                    <td class="flex items-center px-6 py-4 space-x-3">
                      @canany(['department create'])
                      <button class="text-sm border border-gray-300 dark:border-gray-500 dark:hover:border-gray-400 hover:border-gray-400 px-1 py-0.5 rounded-lg" data-modal-toggle="popup-modal-restore{{ $department->id }}" data-modal-target="popup-modal-restore{{ $department->id }}">
                        {{ __('Restore') }}
                      </button>
                      @endcanany
                      @canany(['department delete'])
                      <button class="text-sm border border-gray-300 dark:border-gray-500 dark:hover:border-gray-400 hover:border-gray-400 px-1 py-0.5 rounded-lg" data-modal-toggle="popup-modal-destroy{{ $department->id }}" data-modal-target="popup-modal-destroy{{ $department->id }}">
                        {{ __('Destroy') }}
                      </button>
                      @endcanany
                    </td>
    ...
    <div class="relative w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-6 text-center">
          ...
          <h3 class="mb-5 font-normal text-gray-500 dark:text-gray-400"><span class="font-bold uppercase text-yellow-500">Warning</span>: This action will restore archived data. Are you sure you want to restore the data with the name "<span class="font-bold underline text-gray-700 dark:text-gray-200">{{ $department->name }}</span>"?</h3>
          <div class="inline-flex w-full">
            <div class="basis-1/2">
              <a href="{{ route('department.restore', $department->id) }}" class="text-gray-700 w-full dark:text-gray-300 hover:bg-green-700 dark:hover:bg-green-800 hover:text-white dark:hover:text-white focus:ring-4 focus:outline-none focus:ring-green-500 dark:focus:ring-green-800 rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 mb-2">
                Yes, I'm sure
              </a>    
            </div>
            <div class="basis-1/2">
              <button data-modal-hide="popup-modal-restore{{ $department->id }}" type="button" class="text-gray-700 w-full dark:text-gray-300 bg-gray-300 hover:bg-gray-400 focus:ring-4 focus:ring-gray-300 rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-600 dark:hover:bg-gray-800 focus:outline-none dark:focus:ring-gray-800">No, cancel</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-6 text-center">
          ...
          <div class="inline-flex">
            ...
            <button data-modal-hide="popup-modal-destroy{{ $department->id }}" type="button" class="text-gray-700 dark:text-gray-300 bg-gray-300 hover:bg-gray-400 focus:ring-4 focus:ring-gray-300 rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-600 dark:hover:bg-gray-800 focus:outline-none dark:focus:ring-gray-800">No, cancel</button>
          </div>
        </div>
      </div>
    </div>
