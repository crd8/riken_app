            ...
            <div>
              <header class="max-w-xl mb-7 mt-5">
                ...
                <p class="text-sm text-gray-700 dark:text-gray-300">
                  {{ __('The process of editing area in a system or app.') }}
                </p>
              </header>
            </div>
            <form class="max-w-xl" action="{{ route('area.update', $area->id) }}" method="POST">
              @csrf
              @method('PUT')
              <div class="mt-6">
                <label for="code" class="block mb-1.5 text-sm text-gray-800 dark:text-gray-200">Area Code</label>
                <input type="text" id="code" name="code" value="{{ old('code', $area->code) }}" class="dark:bg-gray-700 text-sm w-6/12 text-gray-700 dark:text-gray-200 py-2 border-b-2 border-gray-400/30 dark:border-gray-600/30 border-b-gray-400 dark:border-b-gray-500 rounded focus:ring-0 focus:border-t-gray-400/30 focus:border-b-2 focus:border-b-sky-600 dark:focus:border-b-gray-200 focus:border-x-gray-400/30" required autofocus>
              ...
              <div class="mt-6">
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $area->name) }}" class="dark:bg-gray-700 text-sm w-10/12 text-gray-700 dark:text-gray-200 py-2 border-b-2 border-gray-400/30 dark:border-gray-600/30 border-b-gray-400 dark:border-b-gray-500 rounded focus:ring-0 focus:border-t-gray-400/30 focus:border-b-2 focus:border-b-sky-600 dark:focus:border-b-gray-200 focus:border-x-gray-400/30" required>
                @foreach ($errors->get('name') as $error)
                  <span class="block text-xs mt-0.5 text-red-600 dark:text-red-700">{{ $error }}</span>
                @endforeach
              </div>
              <div class="mt-6">
                <label for="address" class="block mb-1.5 text-sm text-gray-800 dark:text-gray-200">Address</label>
                <textarea name="address" id="address" rows="4" class="dark:bg-gray-700 text-sm w-10/12 text-gray-700 dark:text-gray-200 py-2 border-b-2 border-gray-400/30 dark:border-gray-600/30 border-b-gray-400 dark:border-b-gray-500 rounded focus:ring-0 focus:border-t-gray-400/30 focus:border-b-2 focus:border-b-sky-600 dark:focus:border-b-gray-200 focus:border-x-gray-400/30" required>{{ old('address', $area->address) }}</textarea>
