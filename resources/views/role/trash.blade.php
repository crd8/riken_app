<x-app-layout>
                ...
                <tbody>
                  ...
                  <tr class="border-b border-gray-500/30 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200/70 dark:hover:bg-gray-800/60">
                    ...
                    <td class="flex items-center px-6 py-3 whitespace-nowrap space-x-3">
                      @canany(['role create'])
                      <button class="text-sm border border-gray-500/40 dark:border-gray-500 dark:hover:border-gray-400 hover:border-gray-400 px-1 py-0.5 rounded-lg" data-modal-toggle="popup-modal-restore{{ $role->id }}" data-modal-target="popup-modal-restore{{ $role->id }}">
                        {{ __('Restore') }}
                      </button>
                      @endcanany
                      @canany(['role delete'])
                      <button class="text-sm border border-gray-500/40 dark:border-gray-500 dark:hover:border-gray-400 hover:border-gray-400 px-1 py-0.5 rounded-lg" data-modal-toggle="popup-modal-destroy{{ $role->id }}" data-modal-target="popup-modal-destroy{{ $role->id }}">
                        {{ __('Delete') }}
                      </button>
                      @endcanany
                    </td>
  ...
  {{-- Modal --}}
  @foreach ($roles as $role)
  <div id="popup-modal-restore{{ $role->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full box-border">
    <div class="relative w-full max-w-xl max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <div>
          <div class="p-8">
            <h1 class="text-gray-800 dark:text-gray-300 text-md font-semibold mb-2">Restore this <span class="font-bold underline text-gray-950 dark:text-gray-200">{{ $role->name }}</span>?</h1>
  ...
  <div id="popup-modal-destroy{{ $role->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-xl max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <div>
          <div class="p-8">
            <h1 class="text-gray-800 dark:text-gray-300 text-md font-semibold mb-2">Delete this <span class="font-bold underline text-gray-950 dark:text-gray-200">{{ $role->name }}</span>?</h1>
          ...
          <div class="text-center">
            <div class="bg-gray-100 dark:bg-gray-700 border-t dark:border-t-gray-600 rounded-b-md">
              <div class="py-3 inline-flex w-full mt-3 gap-4 px-8">
                ...
                <div class="basis-1/2">
                  <button data-modal-hide="popup-modal-destroy{{ $role->id }}" type="button" class="w-full border border-gray-500/40 text-gray-700 dark:text-gray-200 hover:text-sky-600 dark:hover:text-sky-50 bg-gray-50 dark:bg-gray-600 focus:ring-2 ring-offset-2 ring-offset-gray-200 dark:ring-offset-gray-900 focus:ring-sky-500 rounded text-sm font-semibold px-5 py-2 mb-2 focus:outline-none dark:focus:ring-gray-500 focus:transition focus:ease-in focus:duration-75">No, cancel</button>
                </div>
              </div>
            </div>
          </div>
