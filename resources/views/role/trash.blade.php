<x-app-layout>
              ...
              <div>
                <header class="max-w-xl">
                  <h2 class="text-gray-600 dark:text-gray-200">
                    {{ __('List of deleted Roles') }}
                  </h2>
                  <p class="mt-1 text-sm text-gray-400 dark:text-gray-400">
                    {{ __('This list lists all the roles are deleted in the system.') }}
                  </p>
                </header>
              </div>
              <form class="w-4/12" action="{{ route('role.trash') }}" method="GET">   
              ...
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                  <tr>
                    ...
                    <th scope="col" class="px-6 py-3">
                      <a href="{{ route('role.trash', ['order' => ($order === 'oldest' ? 'latest' : 'oldest'), 'page' => $roles->currentPage()]) }}">
                ...
                <tbody>
                  ...
                  <tr class="bg-gray-100 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200/70 dark:hover:bg-gray-800/60">
                    ...
                    <td class="px-6 py-4">
                      {{ Carbon\Carbon::parse($role->deleted_at)->format('l, d F Y, H:i A') }}
                    </td>
  ...
  {{-- Modal --}}
  @foreach ($roles as $role)
  <div id="popup-modal-restore{{ $role->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal-restore{{ $role->id }}">
          <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
          </svg>
        ...
        <div class="p-6 text-center">
          ...
          <h3 class="mb-5 font-normal text-gray-500 dark:text-gray-400"><span class="font-bold uppercase text-yellow-500">Warning</span>: This action will restore archived data. Are you sure you want to restore the data with the name "<span class="font-bold underline text-gray-700 dark:text-gray-200">{{ $role->name }}</span>"?</h3>
          <div class="inline-flex">
            <a href="{{ route('role.restore', $role->id) }}" class="text-gray-700 dark:text-gray-300 hover:bg-green-700 dark:hover:bg-green-800 hover:text-white dark:hover:text-white focus:ring-4 focus:outline-none focus:ring-green-500 dark:focus:ring-green-800 rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2 mb-2">
              Yes, I'm sure
            </a>    
            <button data-modal-hide="popup-modal-restore{{ $role->id }}" type="button" class="text-gray-700 dark:text-gray-300 bg-gray-300 hover:bg-gray-400 focus:ring-4 focus:ring-gray-300 rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-600 dark:hover:bg-gray-800 focus:outline-none dark:focus:ring-gray-800">No, cancel</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div id="popup-modal-destroy{{ $role->id }}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal-destroy{{ $role->id }}">
          <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
          </svg>
          <span class="sr-only">Close modal</span>
        ...
        <div class="p-6 text-center">
          ...
          <h3 class="mb-5 font-normal text-gray-500 dark:text-gray-400"><span class="font-bold uppercase text-yellow-500">Warning</span>: This action will permanently delete the data. Are you sure you want to delete the data with the name "<span class="font-bold underline text-gray-700 dark:text-gray-200">{{ $role->name }}</span>"?</h3>
          <div class="inline-flex">
            <form method="POST" action="{{ route('role.destroy-permanently', $role->id) }}">
              @csrf
              @method('DELETE')
            ...
            <button data-modal-hide="popup-modal-destroy{{ $role->id }}" type="button" class="text-gray-700 dark:text-gray-300 bg-gray-300 hover:bg-gray-400 focus:ring-4 focus:ring-gray-300 rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-gray-600 dark:hover:bg-gray-800 focus:outline-none dark:focus:ring-gray-800">No, cancel</button>
          </div>
        </div>
      </div>
    </div>
